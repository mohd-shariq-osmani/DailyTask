...
class DailyTaskWidgetProvider : AppWidgetProvider() {
    ...
    companion object {
        const val ACTION_TOGGLE_TASK = "com.dailytask.app.ACTION_TOGGLE_TASK"
        const val EXTRA_TASK_ID = "com.dailytask.app.EXTRA_TASK_ID"
        ...
        if (intent.action == ACTION_TOGGLE_TASK) {
            ...
            if (taskId != -1L) {
                CoroutineScope(Dispatchers.IO).launch {
                    val db = DailyTaskDatabase.getDatabase(context)
                    val dao = db.taskDao()
    ...
    private fun updateAppWidget(
        ...
        // Use background thread to query the DB and update the UI elements
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val db = DailyTaskDatabase.getDatabase(context)
                val rawTasks = db.taskDao().getAllTasks()
                // Move completed tasks to the bottom of the widget list, preserving their relative display order
                val tasks = rawTasks.sortedWith(compareBy<Task> { it.isCompleted }.thenBy { it.displayOrder })
